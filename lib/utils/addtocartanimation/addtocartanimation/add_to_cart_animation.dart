...
class _AddToCartAnimationState extends State<AddToCartAnimation> {
  ...
  @override
  void initState() {
    widget.receiveCreateAddToCardAnimationMethod(runAddToCardAnimation);
    super.initState();
  }
  ...
  Future<void> runAddToCardAnimation(GlobalKey gkImageContainer) async {
    ...
    animationModel.imageDestPoint = Offset(widget.gkItem.globalPaintBounds!.top,
        widget.gkItem.globalPaintBounds!.left);

    animationModel.imageDestSize = Size(
        widget.gkItem.currentContext!.size!.width,
        widget.gkItem.currentContext!.size!.height);
    ...
    // Improvement/Suggestion 4.3: runCartAnimation is running independently, using gkItem.currentState(main.dart)
    // await this.widget.gkItem.currentState!.runCartAnimation();
